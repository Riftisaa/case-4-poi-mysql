var currentMarker; // Variabel global untuk menyimpan marker yang sedang digeser
var currentPopupContent; // Variabel global untuk menyimpan konten popup dari marker
...
// Hapus event listener 'moveend'
map.off("moveend");

// Fungsi untuk memunculkan modal update POI
function showUpdateModal(marker, popupContent) {
  // Mengisi nilai input dengan data dari marker
  $("#nama_lokasi").val(marker.nama_lokasi);
  $("#deskripsi_lokasi").val(marker.deskripsi_lokasi);
...
// Event listener untuk saat marker digeser
...
$("#updatePOI").on("click", function () {
  ...
  $.ajax({
    ...
    success: function (response) {
      console.log(response);
      currentMarker.bindPopup(currentPopupContent).openPopup();
      read_maps();
    },
    error: function (xhr, status, error) {
      console.error("Terjadi kesalahan:", error);
